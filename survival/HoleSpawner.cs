...
public partial class HoleSpawner : Node2D {
    ...
    private void SpawnBulletHole() {
        CreateCrawler(new Vector2((float)GD.RandRange(0, GetViewportRect().Size.X), -200),
            0, Mathf.Pi / 2 + (float)GD.RandRange(-0.75, 0.75), 150, bulletShape, 10.0f, true);
    }
    ...
    private void SpawnGroundHole() {
        ...
        CreateCrawler(pos, rotation, direction, 150, groundHoleShape, 10.0f, false);
    }

    private CrawlingTerrain CreateCrawler(Vector2 position, float rotation, float direction, float speed, Shape2D shape,
        float lifeTime, bool showGradient) {
        ...
        crawler.ShowGradientTexture = showGradient;
